...
/**
 * Подсказка к отказу загрузки — только когда журнал сам причину не назвал.
 *
 * Приписывать «база занята, проверьте пароль» к любому отказу нельзя: платформа
 * часто отвечает совершенно определённо, и советы поверх такого ответа уводят
 * в сторону. Живой случай 27.08.2026: журнал сказал «Файл не обнаружен
 * …\Ext\ParentConfigurations\<Имя>.cf», 1С при этом была закрыта, а подсказка
 * звала искать чужие сеансы.
 */
export function loadHint(designerLog) {
  const text = String(designerLog || '');
  if (/не обнаружен|не удается найти|не удаётся найти|не найден/i.test(text)) {
    return '\nПлатформа не нашла файл, который ждала в выгрузке. Каталог выгрузки неполон: '
      + 'проверьте, что его не чистили сторонние средства, и повторите объединение.';
  }
  if (/монопольн|заблокирован|занята/i.test(text)) {
    return '\nБаза занята: закройте все сеансы 1С:Предприятия и конфигуратора, '
      + 'загрузка требует монопольного доступа.';
  }
  if (/пароль|идентифицирован|прав|доступ/i.test(text)) {
    return '\nПохоже на отказ по правам: проверьте имя пользователя, пароль '
      + 'и наличие права «Администрирование».';
  }
