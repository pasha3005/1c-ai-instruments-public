 ...
 *
 * Проверено на реальной ERP (14 391 объект, поставщик «Интегратор»):
...
export async function compareWithVendorInBase({
  ...
  for (const reportType of ['Full', 'Brief']) {
    ...
    if (!(await pathExists(reportFile))) {
      ...
      verdict = classifyFailure(designerLog);
      failure = verdict.reason;
      // Отказ «поставщика в базе нет» и несовпадение версии платформы вторая
      // попытка не исправит: вид отчёта тут ни при чём. Незачем ждать ещё раз.
      if (verdict.vendorUnavailable || verdict.needsNewerPlatform) break;
      continue;
    }
...
/**
 * Разбирает отказ конфигуратора: почему сравнения с поставщиком не вышло.
 *
 * Разница принципиальна для того, что показать человеку.
 *
 * **«Конфигурация 'Конфигурация поставщика' недоступна»** — платформа
 * не отдаёт конфигурацию поставщика как операнд сравнения. Что про этот ответ
 * известно точно, по опытам на демо-базе УНФ 3.0.13.374 (11.08.2026):
 *
 *  * он **не означает, что конфигурация снята с поддержки**. На той базе
 *    «Настройка поддержки» показывает «Конфигурация находится на поддержке»,
 *    поставщик «Фирма "1С"», версия 3.0.13.374 — и всё равно отказ. Сначала
 *    решили обратное и ошиблись, поправил пользователь;
 *  * дело **не в аргументах**: ответ одинаков с именем конфигурации
 *    в `-SecondName`, с чужим именем, без этого параметра вовсе, с добавленными
 *    `-SecondVersion` и `-SecondVendor`, при перестановке сторон и при
 *    сравнении с `DBConfiguration` вместо основной — семь форм вызова;
 *  * дело **не в базе, платформе и не в правах**: на той же базе тем же
 *    вызовом сравнение основной конфигурации с конфигурацией базы данных
 *    проходит с кодом 0, а на базе ERP сравнение с поставщиком отдаёт отчёт
 *    на 231 КБ;
 *  * в самом окне «Настройка поддержки» кнопка **«Сравнить, объединить»
 *    неактивна**, а «Сохранить в файл» — активна. То есть сравнивать
 *    с поставщиком отказывается и конфигуратор, а выгрузить поставку в файл
 *    он готов. Это и есть путь для пользователя.
 *
 * Поэтому текст ответа — «платформа не отдаёт поставщика», а совет — сохранить
 * поставку в файл из того же окна; для снятой с поддержки останется дистрибутив
 * релиза, который программа ищет сама.
 *
 * **«Требуется более новая версия платформы»** — выбрана платформа старше той,
 * которой сделана база. Лечится выбором версии на форме, и сказать об этом
 * надо прямо, а не прятать за «не удалось сравнить».
 *
 * @param {string} designerLog содержимое файла /Out
 */
export function classifyFailure(designerLog) {
  const text = String(designerLog || '').trim();

  if (/Конфигурация\s*'Конфигурация поставщика'\s*недоступна/i.test(text)) {
    return {
      vendorUnavailable: true,
      reason: 'платформа не отдаёт конфигурацию поставщика для сравнения — отвечает '
        + '«Конфигурация \'Конфигурация поставщика\' недоступна». Дело не в аргументах команды: '
        + 'ответ одинаков при любом их наборе, а сравнение основной конфигурации '
        + 'с конфигурацией базы данных на той же базе проходит.',
    };
  }
  ...
  if (/требуется более новая версия платформы/i.test(text)) {
    return {
      ...
      reason: 'база сделана более новой версией платформы, чем выбранная. '
        + 'Укажите версию платформы на форме — ту, которой открывается эта база.',
    };
  }
 ...
 */
/*
 * Стороны сравнения платформа называет по-разному, и это не мелочь.
 * При сравнении с поставщиком — «в основной конфигурации» и «в конфигурации
 * поставщика». При сравнении ДВУХ ФАЙЛОВ (так строятся правки помещений
 * в режиме только чтения) — «в первой конфигурации» и «во второй». Формулировка
 * снята с живого отчёта 13.08.2026; без второго варианта блоки строк не
 * распознавались вовсе, и правки помещений выходили пустыми при полностью
 * успешном сравнении.
 */
const CLIENT_ONLY_LINES = /только в (?:основной|первой) конфигурации:\s*(\d+)\s*-\s*(\d+)$/i;
const CHANGED_LINES = /^Изменено:\s*(\d+)\s*-\s*(\d+)$/i;
const VENDOR_ONLY_LINES = /только (?:в конфигурации поставщика|во второй конфигурации):\s*(\d+)\s*-\s*(\d+)$/i;
 ...
 *    в кавычках подряд;
 *  * блок «Изменено» — обе версии: «<» основная конфигурация, «>» поставщик;
...
function readModuleLines(moduleLines, key, line, state) {
  ...
  // В блоке вставки строка кода идёт в кавычках, версии поставщика у неё нет —
  // это чистое добавление.
  //
  // В блоке «Изменено» печатаются обе версии, и порядок такой же, как в обычном
  // diff: «<» — ПЕРВАЯ сравниваемая конфигурация, то есть основная (наша),
  // «>» — вторая, конфигурация поставщика. Было наоборот, и это давало сразу
  // две ошибки: в отчёте колонки менялись местами (в «поставщике» оказывался
  // код с пометкой разработчика «// ++ ПРО_инт_и_…», которой у поставщика быть
  // не может — на это указал пользователь), а участок правки искался в модуле
  // по строкам ПОСТАВЩИКА. Найтись они могли только там, где обе конфигурации
  // совпадают, — то есть правилам открывался типовой код вендора.
