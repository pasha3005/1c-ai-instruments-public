...
export async function compareWithVendorInBase({
  ...
  for (const reportType of ['Full', 'Brief']) {
    ...
    if (!(await pathExists(reportFile))) {
      ...
      if (verdict.offSupport || verdict.needsNewerPlatform) break;
      continue;
    }
 ...
 *
 * **«Конфигурация 'Конфигурация поставщика' недоступна»** — конфигурации
 * поставщика в базе нет вовсе, то есть конфигурация снята с поддержки. Это
 * не наша беда и не ошибка настроек: её оттуда не достанет и разработчик
 * в конфигураторе, брать нечего. Проверено на демо-базе УНФ 3.0.13.374:
 * ответ один и тот же с именем конфигурации в `-SecondName`, с чужим именем
 * и вообще без этого параметра, за 5 секунд. Та же команда на базе ERP,
 * стоящей на поддержке, отдаёт отчёт на 231 КБ и код 0 — значит дело в базе,
 * а не в команде. Поэтому советовать «Конфигуратор → Поддержка → Сохранить
 * конфигурацию поставщика в файл» здесь нельзя: сохранять тоже нечего.
 *
...
export function classifyFailure(designerLog) {
  ...
  if (/Конфигурация\s*'Конфигурация поставщика'\s*недоступна/i.test(text)) {
    return {
      offSupport: true,
      reason: 'конфигурация снята с поддержки — конфигурации поставщика в этой базе нет. '
        + 'Платформа отвечает «Конфигурация \'Конфигурация поставщика\' недоступна»: сравнивать '
        + 'не с чем, и достать её из базы не получится ни у программы, ни руками в конфигураторе.',
    };
  }
