...
/**
 * Убирает первое помещение хранилища — версию 1.
 *
 * При создании хранилища в него попадает ВСЯ конфигурация целиком, поэтому
 * версия 1 — это не доработка, а снимок исходного состояния: десятки тысяч
 * объектов с одним автором и комментарием вроде «Создание хранилища
 * конфигурации». В отчёте о доработках ей делать нечего, и на неё же уходит
 * основное время прогона. Заметно это, когда период не указан и берётся вся
 * история, но версия 1 бесполезна в любом периоде — поэтому она исключается
 * всегда и одинаково у конфигурации и у расширений.
 *
 * Опознаётся по НОМЕРУ, а не по комментарию: комментарий пишет человек,
 * и он бывает любым.
 *
 * @param {object[]} commits
 */
export function dropInitialCommit(commits) {
  return (commits || []).filter((commit) => Number(commit.version) !== 1);
}
