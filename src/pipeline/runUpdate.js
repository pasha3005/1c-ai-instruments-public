...
import { runUpdateHandlers } from './updateHandlers.js';
...
async function runPipeline({ updateId, input, progress }) {
  ...
  try {
    ...
    // ---------- 4. Каким путём обновлять ----------
    // Пять секунд до всего дорогого. Вопрос здесь один и ставится ровно так,
    // как на него может ответить платформа: ДАЁТ ЛИ ОНА СРАВНИТЬ конфигурацию
    // с конфигурацией поставщика. Даёт — идём объединением: сравнивать есть
    // с чем, а значит есть и о чём говорить. Не даёт («Конфигурация
    // 'Конфигурация поставщика' недоступна») — объединять нечем, и обновление
    // делает сама платформа своей же командой /UpdateCfg.
    //
    // Утверждать по этому ответу «доработок нет» нельзя, и раньше этап так
    // и назывался — «Есть ли в базе доработки», хотя проверял он не это.
    // Отчего зависит доступность операнда, вендор не объясняет; разобрано
    // в скилле onec-vendor-diff.
    startStage('detect', 'Спрашиваем платформу, даст ли она сравнить с поставщиком');
    ...
    // Вход в базу не удался — дальше идти незачем: теми же именем и паролем
    // программа будет выгружать конфигурацию и записывать результат. Раньше
    // такой ответ попадал в «неясно» и прогон шёл дальше, где вставал уже
    // на запросе пароля (живой случай 27.08.2026).
    if (presence.authFailed) {
      throw new Error(presence.reason
        || 'Войти в информационную базу не удалось: платформа не приняла имя пользователя '
          + 'или пароль. Проверьте их на форме.');
    }

    if (presence.present === false) {
      progress.done('detect', 'конфигурации поставщика в базе нет — обновляет платформа');
    ...
    if (presence.present === null) {
      // Ответ не из известных. Идём объединением — это осторожный путь:
      // он сохраняет доработки, если они есть, и ничего не пишет в базу
      // без подтверждения. Но человек должен видеть, ЧТО ответила платформа.
      const answer = firstLine(presence.log) || presence.reason || 'ответа нет';
      progress.done('detect', `платформа ответила: ${answer.slice(0, 80)}`);
      progress.message(
        'Платформа ответила на проверку не так, как обычно: '
        + `«${answer}». Обычных ответов два — «Не указано имя конфигурации поставщика» `
        + '(поставщик в базе есть, идём объединением) и «Конфигурация \'Конфигурация '
        + 'поставщика\' недоступна» (сравнивать не с чем, обновляет платформа). Раз ответ '
        + 'другой, выбран более осторожный путь — объединение: оно сохранит доработки, '
        + 'если они есть, и ничего не запишет в базу без вашего подтверждения.',
        'warn',
      );
    } else {
...
async function runTypical({
  ...
  progress.message(
    'Платформа не даёт сравнить эту конфигурацию с конфигурацией поставщика: она отвечает, '
    + 'что конфигурация поставщика недоступна. Так отвечает база, в которой возможность '
    + 'изменения не включали, — объединять в ней нечего и не с чем. Обновление выполнит сама '
    + 'платформа, как это делает конфигуратор командой «Обновить конфигурацию»; если доработки '
    + 'в базе всё-таки есть, сравнение после обновления это покажет, и в отчёте будет '
    + 'предупреждение.',
  );
...
/**
 * Первая содержательная строка ответа платформы.
 *
 * Журналы конфигуратора многострочны и начинаются со служебных строк; в подпись
 * этапа идёт первая, в которой есть что читать.
 */
