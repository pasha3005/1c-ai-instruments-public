...
function renderModeNote(result) {
  ...
  <div class="callout callout--good">
    ...
    ${result.vendorAhead ? `<br>Выполнено и второе сравнение — конфигурации поставщика
    с файлом новой поставки: в новом релизе поставщик изменил
    ${formatNumber(result.vendorAhead.modified)}, добавил
    ${formatNumber(result.vendorAhead.added)} и убрал
    ${formatNumber(result.vendorAhead.removed)} объектов. Им и проверяется, менял ли поставщик
    то же свойство, что и вы.` : ''}
    ${stats.fromTarget ? `<br>Ещё по ${plural(stats.fromTarget, 'описанию', 'описаниям', 'описаниям')}
    объектов точка отсчёта взята из нового релиза: поставщик их не менял, значит в текущей
    поставке они были такими же.` : ''}
...
function renderAuto(result) {
  ...
  <p class="section__lead">
    ...
    сама. Каждое место показано полностью: три исходные версии, то, что записано в файл,
    и объяснение, почему выбран именно такой вариант. <b>Каждое из них ждёт вашего
    подтверждения</b> в окне «Разобрать спорные места»: до этого запись в информационную базу
    не предлагается. Там же место правится, и там же лежит кнопка возврата к варианту программы.
  </p>
...
function renderNext(result) {
  ...
  if (manual) {
    steps.push(
      'Разберите места из раздела «Требуют вашего решения» — кнопкой <b>«Разобрать спорные '
      + 'места»</b> на странице обновления. Там то же окно, что и в конфигураторе: слева дерево, '
      + 'сверху ваш код и код новой поставки, снизу результат, который правится и сохраняется '
      + 'прямо там. <b>Пока хоть одно место не разобрано, загрузка в информационную базу '
      + 'не предлагается и не выполняется.</b> Те же файлы можно править и вручную — в выгрузке '
      + `(<span class="mono">${esc(result.mergedDir || '')}</span>) или в каталоге `
      + 'со спорными файлами.',
    );
  }
  if (!result.loaded && result.mode === 'typical') {
    ...
    steps.push(
      (manual ? 'Разобрав спорные места, загрузите' : 'Загрузите')
      + ' объединённую выгрузку в основную конфигурацию — кнопкой «Загрузить '
