...
function renderScopeNote(result) {
  ...
  // Единственная причина, по которой может быть проверен типовой код, —
  // недоступная конфигурация поставщика. Отдельного «по требованию заказчика»
  // больше нет: флаг убран, типовой код намеренно не проверяется никогда.
  if (!scope?.filtered) {
    return `
  <div class="callout callout--warn">
