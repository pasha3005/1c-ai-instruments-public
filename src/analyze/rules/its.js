 ...
 *
 * Раньше признаком служили «строка кончается точкой с запятой» и «строка
 * начинается ключевым словом языка». Оба давали ложные замечания на обычную
 * прозу разработчика (замечание пользователя, 20.08.2026):
 *
 *   // Для показателей, зависящих по шкале от стажа…   ← начинается с «Для»
 *   //   * Ключ - Строка - имя реквизита в шаблоне;    ← кончается на «;»
 *   // Если хотя бы у одной статьи указан 20 счет,     ← начинается с «Если»
 *
 * Поэтому ищется СИНТАКСИС целиком, а не первое слово: присваивание, вызов
 * метода, законченная конструкция языка. Проза под такие шаблоны не подходит:
 * «Если …» без «Тогда» и «Для …» без «Цикл» кодом не являются.
 */
const CODE_LIKE_PATTERNS = [
  // Присваивание: «Сумма = Цена * 2;», «Объект.Реквизит = Значение;»
  /^[A-Za-zА-Яа-яЁё_][\wА-Яа-яЁё.[\]"]*\s*=\s*\S.*;\s*$/,
  // Вызов метода: «ЗаполнитьСвойства(Объект);», «Модуль.Метод();»
  /^[A-Za-zА-Яа-яЁё_][\wА-Яа-яЁё.]*\s*\(.*\)\s*;\s*$/,
  // Условие и цикл целиком, а не первое слово прозы.
  /^(?:Если|ИначеЕсли|If|ElsIf)\s+.+\s+(?:Тогда|Then)\s*$/i,
  /^(?:Для|Пока|For|While)\s+.+\s+(?:Цикл|Do)\s*$/i,
  // Завершители блоков — сами по себе целые операторы.
  /^(?:КонецЕсли|КонецЦикла|КонецПопытки|Иначе|Попытка|Исключение|EndIf|EndDo|EndTry|Else|Try|Except)\s*;?\s*$/i,
  /^(?:Возврат|Return)(?:\s+.+)?;\s*$/i,
  /^(?:Продолжить|Прервать|Continue|Break)\s*;\s*$/i,
  /^(?:Перем|Var)\s+[A-Za-zА-Яа-яЁё_][\wА-Яа-яЁё,\s]*;\s*$/i,
  // Объявление метода: закомментированная процедура целиком.
  /^(?:Процедура|Функция|Procedure|Function)\s+[A-Za-zА-Яа-яЁё_][\wА-Яа-яЁё]*\s*\(/i,
];

/**
 * Пояснение, которое кодом не является ни при каких шаблонах: описание
 * параметра в стиле БСП («* Ключ - Строка - имя реквизита;») и обычная фраза,
 * где после слова идёт тире с пояснением.
 */
const DOC_COMMENT = /^[*\-\s]*[A-Za-zА-Яа-яЁё_][\wА-Яа-яЁё]*\s+[-–—]\s+/;

function looksLikeCommentedCode(text) {
  if (DOC_COMMENT.test(text)) return false;
  return CODE_LIKE_PATTERNS.some((re) => re.test(text));
}
...
function detectCommentedCode(ctx) {
  ...
  for (const token of comments || []) {
    ...
    const looksLikeCode = looksLikeCommentedCode(text);
...
function detectMissingDataExchangeLoading(ctx) {
  ...
  for (const routine of structure.routines) {
    ...
    // Стандарт назван по тому обработчику, который найден: приписывать
    // ОбработкеПроведения заголовок стандарта про ПередЗаписью и ПриЗаписи —
    // неправда, хотя требование к ней то же самое.
    const ofWriting = name === 'передзаписью' || name === 'призаписи';

    ctx.report({
      ruleId: 'std.data-exchange-loading',
      title: `Обработчик «${routine.name}» без проверки ОбменДанными.Загрузка`,
      // Заголовок группы в отчёте общий: в одной группе лежат разные
      // обработчики, и имя первого из них было бы подписью ко всем.
      groupTitle: 'Обработчик записи или проведения без проверки ОбменДанными.Загрузка',
      severity: SEVERITY.MEDIUM,
      category: CATEGORY.STANDARDS,
      line: routine.startLine,
      detail: (ofWriting
        ? `${std('464, 465', 'Обработчики событий ПередЗаписью и ПриЗаписи')}`
        : `${std('464, 465', 'Обработчики событий ПередЗаписью и ПриЗаписи')}то же требование `
          + 'действует в обработчиках проведения и удаления: ')
