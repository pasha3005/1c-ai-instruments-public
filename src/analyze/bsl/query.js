...
/**
 * Источник запроса — объект метаданных, а не временная таблица.
 *
 * Выборка без условия отбора опасна только из таблицы БАЗЫ: «ВЫБРАТЬ … ИЗ
 * Справочник.Номенклатура» без ГДЕ читает справочник целиком. Временная
 * таблица (ВТПоступления, ВремТабл), параметр-таблица (&Таблица) и результат
 * предыдущего пакета уже отобраны выше по тексту запроса — требовать у них
 * ГДЕ бессмысленно (замечание пользователя, 20.08.2026).
 *
 * Признак один и надёжный: имя таблицы БАЗЫ начинается с вида объекта
 * метаданных и содержит точку — «Документ.РеализацияТоваров»,
 * «РегистрНакопления.Продажи.Обороты».
 */
export function isMetadataTable(name) {
  const text = String(name || '').trim();
  if (!text || text.startsWith('&')) return false;
  const at = text.indexOf('.');
  if (at <= 0) return false;
  return METADATA_PREFIXES.has(normalizeName(text.slice(0, at)));
}
...
/**
 * Условие сравнивает цепочку с ПРЕДОПРЕДЕЛЁННЫМ значением.
 *
 * «ПлановыеНачисления.Начисление.ВидВремени В (ЗНАЧЕНИЕ(Перечисление…))» —
 * отбор по значению перечисления или предопределённому элементу. Пользователь
 * прямо сказал 20.08.2026, что такую запись замечанием считать не нужно:
 * на проекте она принята, и разбирать её как ошибку — шум.
 *
 * Проверяется ХВОСТ условия сразу за цепочкой: до ближайшего логического
 * оператора или конца строки должно встретиться ЗНАЧЕНИЕ(…).
 */
function comparedWithPredefined(text, from) {
  const tail = String(text).slice(from, from + 200);
  const stop = tail.search(/(?<![А-Яа-яЁёA-Za-z])(И|ИЛИ|AND|OR)(?![А-Яа-яЁёA-Za-z])/i);
  const part = stop >= 0 ? tail.slice(0, stop) : tail;
  return /(?<![А-Яа-яЁёA-Za-z])(ЗНАЧЕНИЕ|VALUE)\s*\(/i.test(part);
}
...
export function findNonIndexableFilters(text) {
  ...
  const dotted = [...wherePart.matchAll(dottedRe)]
    .filter((m) => !isMetadataChain(m[0]))
    .filter((m) => !comparedWithPredefined(wherePart, m.index + m[0].length));
  if (dotted.length) {
    issues.push({
      kind: 'dottedFilter',
