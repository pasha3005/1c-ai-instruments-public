...
/**
 * Сервер работает старым кодом, а страница уже новая.
 *
 * Node загружает свои модули один раз, при запуске, а `web/` читает с диска
 * на каждый запрос. Обновив каталог программы и не перезапустив её, человек
 * получает новый интерфейс поверх старого сервера — и правки, которым нужен
 * сервер, молча не работают. Живой случай 28.08.2026: пропали кнопки
 * «Показать все» и «Показать весь модуль», потому что старый сервер
 * не отдавал списка методов модуля.
 *
 * Молчать об этом нельзя: со стороны это выглядит как «программа сломалась».
 */
function warnIfStale(serverVersion) {
  const page = document.querySelector('meta[name="app-version"]')?.content;
  if (!page || !serverVersion || page === serverVersion) return;

  const box = $('#appStale');
  if (!box) return;
  box.hidden = false;
  box.textContent = `Программа обновлена до ${page}, но работает ещё ${serverVersion}: `
    + 'закройте это окно и запустите программу заново — часть возможностей '
    + 'до перезапуска недоступна.';
}
